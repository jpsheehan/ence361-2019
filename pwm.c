...
// Constants
// PWM configuration
static const int PWM_RATE = 200;                        //Main (and tail) rotor 200Hz
//static const int PWM_TAIL_RATE = 200;                 //Tail rotor 200Hz
static const int PWM_DIVIDER_CODE = SYSCTL_PWMDIV_8;    //For 40MHz system clk
...
//Variables
/**
 * Duty cycle of main rotor.
 */
static int8_t g_main_duty;

/**
 * Duty cycle of tail rotor.
 */
static int8_t g_tail_duty;

/**
 * Period of the PWM signal stated in number of clock ticks
 */
...
/*********************************************************
 * initialisePWM
 * M0PWM7 (J4-05, PC5) is used for main rotor motor
 * M1PWM5 (J3-10, PF1) is used for tail rotor motor
 *********************************************************/
void pwm_init(void)
{
    // Calculate period (in clock ticks) for given rotor frequency)
    ...
    pwm_set_main_duty(0);       //Main rotor zero duty at start up
